...
public class ResponsableDeCoinServiceImpl implements ResponsableDeCoinService{
	...
	public ResponseEntity<String> saveRespon(ResponsableDeCoin responD) {
		try {
			...
			if (responD.getUser().getUsername().equals(null)) {
		        return new ResponseEntity<>(
		          "Vous devez entrer Votre User name", 
		          HttpStatus.INTERNAL_SERVER_ERROR);//renvoie une erreur 500
		    }
		   else if (responD.getUser().getNumeroTel()==0) {
		        return new ResponseEntity<>(
	...
	@Override
	public ResponseEntity<String> updateRespon(ResponsableDeCoin responD,Long id) {
		Optional<ResponsableDeCoin> resp = responsableDeCoinRepository.findById(id);
		if(resp.isEmpty()) {
			return new ResponseEntity<>(
					"Ce responsable de coin de vente n'existe pas  n'existe pas ",
					HttpStatus.INTERNAL_SERVER_ERROR);//renvoie une erreur 500
		}
		
		responsableDeCoinRepository.saveAndFlush(resp.get());
		return new ResponseEntity<>(
	...
	public List<ResponsableDeCoin> getAllResponsD() {
		return responsableDeCoinRepository.findAll();
	}

	@Override
	public Compte ConsulterSolde(Compte compte) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Transaction Depot(Transaction transaction) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Transaction saveRetrait(Transaction transaction) {
		// TODO Auto-generated method stub
		return null;
	}
